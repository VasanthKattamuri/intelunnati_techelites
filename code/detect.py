...
cap = cv2.VideoCapture('data/training_data.mp4')
    ...
    if ret:
        ...
        for i in range(len(persons)):
            for j in range(i+1,len(persons)):
                if dist(person_centres[i],person_centres[j]) <= distance_threshold:
        ...
        cv2.putText(img,'Count of Violations : '+str(v),(15,frame.shape[0]-10),cv2.FONT_HERSHEY_DUPLEX,1,(255, 255, 0),2)
        writer.write(img)
